...
int main() {
	...
	if (weather_tag_id < 0) {
		std::cerr << "[FAIL] could not parse tag id from get_tag_by_slug response\n";
		++failures;
	} else {
		us::MarketFilter f;
		...
		if (m.has_value() && !m->empty() && m->find("\"slug\"") != std::string::npos) {
			std::cout << "[OK] markets(tag=" << weather_tag_id << ", limit=3) bytes=" << m->size()
					  << '\n';
		} else if (m.has_value()) {
			std::cerr << "[FAIL] markets returned empty payload — tag id " << weather_tag_id
					  << " may be wrong\n";
			++failures;
		} else {
