...
TEST_CASE("polymarket-us-cli --version emits version and exits 0",
          "[us_cli][version]") {
  CliRun r = run_cli("--version", "");
  REQUIRE(r.exit_code == 0);
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("polymarket-us-cli"));
}
...
TEST_CASE("polymarket-us-cli requires --credentials", "[us_cli][args]") {
  ...
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("\"ok\":false"));
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("--credentials"));
}

TEST_CASE("polymarket-us-cli rejects nonexistent credentials path",
          "[us_cli][creds]") {
  CliRun r = run_cli("--credentials /this/path/does/not/exist",
                     R"({"command":"get_account"})");
  REQUIRE(r.exit_code == 2);
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("\"code\":\"auth\""));
}
...
TEST_CASE("polymarket-us-cli rejects empty stdin", "[us_cli][protocol]") {
  ...
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("stdin is empty"));
}

TEST_CASE("polymarket-us-cli rejects malformed stdin JSON",
          "[us_cli][protocol]") {
  const auto creds = make_synthetic_creds_file();
  ...
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("stdin JSON parse"));
}
...
TEST_CASE("polymarket-us-cli rejects unknown command", "[us_cli][protocol]") {
  ...
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("unknown command"));
}
...
TEST_CASE("polymarket-us-cli place_order requires market_id, side, type, "
  ...
  CliRun r = run_cli("--credentials " + creds.string(),
                     R"({"command":"place_order","params":{}})");
  std::filesystem::remove(creds);
  REQUIRE(r.exit_code == 5);
  REQUIRE_THAT(r.stdout_text,
               Catch::Matchers::ContainsSubstring("place_order params"));
}
...
TEST_CASE("polymarket-us-cli cancel_order requires order_id",
  ...
  const auto creds = make_synthetic_creds_file();
  CliRun r = run_cli("--credentials " + creds.string(),
                     R"({"command":"cancel_order","params":{}})");
  std::filesystem::remove(creds);
  REQUIRE(r.exit_code == 5);
  REQUIRE_THAT(r.stdout_text, Catch::Matchers::ContainsSubstring("order_id"));
}

TEST_CASE("polymarket-us-cli get_order requires order_id",
          "[us_cli][get_order][validation]") {
  const auto creds = make_synthetic_creds_file();
  CliRun r = run_cli("--credentials " + creds.string(),
                     R"({"command":"get_order","params":{}})");
  std::filesystem::remove(creds);
  REQUIRE(r.exit_code == 5);
  REQUIRE_THAT(r.stdout_text, Catch::Matchers::ContainsSubstring("order_id"));
}
