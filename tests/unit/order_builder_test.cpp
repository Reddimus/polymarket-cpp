...
TEST_CASE("LimitOrderBuilder", "[clob][order]") {
    Address signer = Address::from_hex("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266");
    Address maker = signer;
    auto token_id = uint256_t::from_uint64(12345);

    SECTION("Valid buy order") {
        auto builder = LimitOrderBuilder::create(signer, maker, SignatureType::EOA, TickSize::Hundredth, 0, false);
...
TEST_CASE("MarketOrderBuilder", "[clob][order]") {
    ...
    SECTION("Valid buy order with USDC amount") {
        auto builder = MarketOrderBuilder::create(signer, maker, SignatureType::EOA, TickSize::Hundredth, 0, false);
        builder.token_id(token_id);
        builder.side(Side::Buy);
