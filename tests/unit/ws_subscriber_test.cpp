...
SubscriberConfig make_test_config() {
  ...
  cfg.secret_key = crypto::base64_encode(
...
TEST_CASE("WS Subscriber configure accepts a valid 64-byte secret",
          "[us][ws][config]") {
  Subscriber sub;
...
TEST_CASE("WS Subscriber configure rejects too-short secret",
          "[us][ws][config]") {
  Subscriber sub;
  SubscriberConfig cfg;
  cfg.key_id = "test";
  ...
  cfg.secret_key = crypto::base64_encode(
      std::span<const std::uint8_t>{short_raw.data(), short_raw.size()});
  auto rc = sub.configure(std::move(cfg));
  REQUIRE_FALSE(rc.has_value());
}

TEST_CASE("WS Subscriber subscribe before connect rejects with NetworkError",
          "[us][ws][config]") {
  Subscriber sub;
...
TEST_CASE("WS Subscriber subscribe with empty slug list is rejected",
          "[us][ws][validation]") {
  Subscriber sub;
  REQUIRE(sub.configure(make_test_config()).has_value());
