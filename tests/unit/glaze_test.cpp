...
TEST_CASE("Glaze parses a representative CLOB book-snapshot payload", "[glaze][clob][parse]") {
...
TEST_CASE("Glaze get_decimal accepts numeric and string-typed prices", "[glaze][clob][parse]") {
...
TEST_CASE("Glaze bool field reads ignore string-typed peers", "[glaze][clob][parse][regression]") {
...
TEST_CASE("Glaze read_json rejects malformed input", "[glaze][clob][parse][error]") {
...
TEST_CASE("Glaze handles missing optional fields as typed defaults", "[glaze][clob][parse]") {
	...
	REQUIRE(get_decimal(root, "missing_decimal").to_string() == polymarket::Decimal{}.to_string());
...
TEST_CASE("Glaze parses a stdin-style command envelope", "[glaze][cli][parse]") {
