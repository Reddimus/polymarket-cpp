...
TEST_CASE("CLOB WebSocketClient with credentials reports authenticated", "[clob][ws][lifecycle]") {
...
TEST_CASE("CLOB WebSocketClient default config matches the documented endpoint",
	...
	WebSocketClient client;
	REQUIRE(client.config().url == "wss://ws-subscriptions-clob.polymarket.com/ws/");
...
TEST_CASE("CLOB authenticated client survives move", "[clob][ws][regression][lifecycle]") {
...
TEST_CASE("CLOB callback registration accepts and replaces handlers", "[clob][ws][callbacks]") {
