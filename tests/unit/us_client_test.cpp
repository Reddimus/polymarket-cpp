...
std::string make_synthetic_secret(std::uint8_t fill = 0x42) {
	...
	return crypto::base64_encode(std::span<const std::uint8_t>{raw.data(), raw.size()});
}

} // namespace

TEST_CASE("Polymarket US set_credentials accepts a 64-byte base64 secret", "[us][auth]") {
	Client client;
	Credentials creds;
...
TEST_CASE("Polymarket US set_credentials rejects a too-short secret", "[us][auth]") {
	Client client;
	Credentials creds;
	creds.key_id = "test";
...
TEST_CASE("Polymarket US set_credentials rejects malformed base64", "[us][auth]") {
	Client client;
	Credentials creds;
	creds.key_id = "test";
...
TEST_CASE("Polymarket US authed call without credentials fails fast", "[us][auth]") {
	Client client;
...
TEST_CASE("Polymarket US get_markets path-build does not infinite-recurse", "[us][regression]") {
...
TEST_CASE("Polymarket US Ed25519 sign-message round trip uses first 32 bytes as seed",
		  "[us][auth]") {
...
TEST_CASE("Polymarket US place_order rejects unknown side without HTTP call",
	...
	REQUIRE_FALSE(result.has_value());
	REQUIRE_THAT(result.error().message(), Catch::Matchers::ContainsSubstring("side"));
}
...
TEST_CASE("Polymarket US get_order_by_id requires credentials", "[us][orders][validation]") {
	...
	REQUIRE_FALSE(result.has_value());
	REQUIRE_THAT(result.error().message(), Catch::Matchers::ContainsSubstring("credentials"));
}
