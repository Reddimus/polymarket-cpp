...
public:
    ...
    Result<void> set_builder_credentials(const BuilderKeyCredentials& credentials);
    ...
    [[nodiscard]] Result<std::vector<PriceResponse>> prices(
        const std::vector<PriceRequest>& requests);

    /// Get midpoint for a token
    [[nodiscard]] Result<MidpointResponse> midpoint(const MidpointRequest& request);

    /// Get midpoints for multiple tokens
    [[nodiscard]] Result<std::vector<MidpointResponse>> midpoints(
        const std::vector<MidpointRequest>& requests);

    /// Get spread for a token
    [[nodiscard]] Result<SpreadResponse> spread(const SpreadRequest& request);

    /// Get spreads for multiple tokens
    [[nodiscard]] Result<std::vector<SpreadResponse>> spreads(
        const std::vector<SpreadRequest>& requests);

    /// Get last trade price for a token
    [[nodiscard]] Result<LastTradePriceResponse> last_trade_price(uint256_t token_id);

    /// Get orderbook for a token
    [[nodiscard]] Result<OrderBookResponse> book(const OrderBookRequest& request);

    /// Get orderbooks for multiple tokens
    [[nodiscard]] Result<std::vector<OrderBookResponse>> books(
        const std::vector<OrderBookRequest>& requests);
    ...
    /// Get price history
    [[nodiscard]] Result<PriceHistoryResponse> price_history(
        const PriceHistoryRequest& request);

    /// Get markets with pagination
    [[nodiscard]] Result<PaginatedResponse<MarketResponse>> markets(
    ...
    /// Sign an order
    [[nodiscard]] Result<SignedOrder> sign_order(
    ...
    [[nodiscard]] Result<std::vector<PostOrderResponse>> post_orders(
        const std::vector<SignedOrder>& orders);

    /// Get an order by ID
    [[nodiscard]] Result<OpenOrderResponse> order(std::string_view order_id);

    /// Get orders with filters
    [[nodiscard]] Result<PaginatedResponse<OpenOrderResponse>> orders(
        const OrdersRequest& request = {},
        std::optional<Cursor> cursor = std::nullopt);

    /// Cancel an order by ID
    [[nodiscard]] Result<CancelOrdersResponse> cancel_order(std::string_view order_id);

    /// Cancel multiple orders
    [[nodiscard]] Result<CancelOrdersResponse> cancel_orders(
    ...
    [[nodiscard]] Result<CancelOrdersResponse> cancel_market_orders(
        std::optional<Bytes32> market = std::nullopt,
        std::optional<uint256_t> asset_id = std::nullopt);

    /// Get trades with filters
    [[nodiscard]] Result<PaginatedResponse<TradeResponse>> trades(
        const TradesRequest& request = {},
        std::optional<Cursor> cursor = std::nullopt);

    /// Get balance and allowance
    [[nodiscard]] Result<BalanceAllowanceResponse> balance_allowance(
        const BalanceAllowanceRequest& request = {});

    /// Update allowance
    [[nodiscard]] VoidResult update_balance_allowance(
        const BalanceAllowanceRequest& request);
    ...
    [[nodiscard]] VoidResult delete_notifications(
        const std::vector<std::string>& notification_ids);

    /// Check if an order is scoring
    [[nodiscard]] Result<OrderScoringResponse> is_order_scoring(std::string_view order_id);

    /// Check if orders are scoring
    [[nodiscard]] Result<std::vector<OrderScoringResponse>> are_orders_scoring(
        const std::vector<std::string>& order_ids);
    ...
    /// Post heartbeat
    [[nodiscard]] Result<HeartbeatResponse> post_heartbeat(
        std::optional<std::string> heartbeat_id = std::nullopt);
