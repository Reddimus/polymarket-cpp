...
protected:
	...
	OrderBuilder(Address signer, Address maker, SignatureType sig_type, TickSize tick_size,
				 std::uint32_t fee_rate, bool neg_risk)
		: signer_(std::move(signer)), maker_(std::move(maker)), signature_type_(sig_type),
		  tick_size_(tick_size), fee_rate_(fee_rate), neg_risk_(neg_risk) {}
...
public:
	...
	static OrderBuilder<Limit> create(Address signer, Address maker, SignatureType sig_type,
		...
		return OrderBuilder<Limit>(std::move(signer), std::move(maker), sig_type, tick_size,
								   fee_rate, neg_risk);
	}
...
public:
	...
	static OrderBuilder<Market> create(Address signer, Address maker, SignatureType sig_type,
									   TickSize tick_size, std::uint32_t fee_rate, bool neg_risk) {
		return OrderBuilder<Market>(std::move(signer), std::move(maker), sig_type, tick_size,
									fee_rate, neg_risk);
	}
