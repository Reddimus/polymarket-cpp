...
constexpr std::uint64_t IEEE754_MAX_SAFE =
    (1ULL << 53) - 1; // For JSON number safety
} // namespace constants
...
template <typename OrderKind> class OrderBuilder {
public:
...
protected:
  ...
  OrderBuilder(Address signer, Address maker, SignatureType sig_type,
      ...
      : signer_(std::move(signer)), maker_(std::move(maker)),
        signature_type_(sig_type), tick_size_(tick_size), fee_rate_(fee_rate),
        neg_risk_(neg_risk) {}
...
/// Limit order builder
template <> class OrderBuilder<Limit> : public OrderBuilder<void> {
public:
  /// Create a new limit order builder
  static OrderBuilder<Limit> create(Address signer, Address maker,
                                    SignatureType sig_type, TickSize tick_size,
                                    std::uint32_t fee_rate, bool neg_risk) {
    return OrderBuilder<Limit>(std::move(signer), std::move(maker), sig_type,
                               tick_size, fee_rate, neg_risk);
  }
...
template <> class OrderBuilder<Market> : public OrderBuilder<void> {
public:
  /// Create a new market order builder
  static OrderBuilder<Market> create(Address signer, Address maker,
                                     SignatureType sig_type, TickSize tick_size,
                                     std::uint32_t fee_rate, bool neg_risk) {
    return OrderBuilder<Market>(std::move(signer), std::move(maker), sig_type,
                                tick_size, fee_rate, neg_risk);
  }
  ...
  /// Set price hint (optional, will be calculated from orderbook if not
  /// provided)
  OrderBuilder &price(Decimal p) {
    price_ = p;
    return *this;
