...
public:
    ...
    [[nodiscard]] Result<SubscriptionId> subscribe_market(
        const std::vector<uint256_t>& asset_ids);

    /// Subscribe to market data with custom features enabled
    [[nodiscard]] Result<SubscriptionId> subscribe_market_with_options(
        const std::vector<uint256_t>& asset_ids,
    ...
    [[nodiscard]] Result<SubscriptionId> subscribe_user(
        const std::vector<Bytes32>& markets = {});
