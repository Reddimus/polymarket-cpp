...
inline constexpr std::string_view EXCHANGE =
    "0x4bFb41d5B3570DeFd03C39a9A4D8dE6Bd8B8982E";
/// NegRisk CTF Exchange on Polygon
inline constexpr std::string_view NEG_RISK_EXCHANGE =
    "0xC5d563A36AE78145C45a50134d48A1215220f80a";
/// Conditional Token Framework on Polygon
inline constexpr std::string_view CTF =
    "0x4D97DCd97eC945f40cF65F87097ACe5EA0476045";
/// USDC on Polygon
inline constexpr std::string_view USDC =
...
struct Order {
  ...
  [[nodiscard]] Keccak256Hash type_hash(bool neg_risk,
                                        std::uint64_t chain_id = 137) const;
};
...
[[nodiscard]] Result<Signature> sign_clob_auth(const PrivateKey &key,
                                               const ClobAuth &auth,
                                               std::uint64_t chain_id = 137);

/// Sign an Order struct for CLOB order placement
[[nodiscard]] Result<Signature> sign_order(const PrivateKey &key,
                                           const Order &order, bool neg_risk,
                                           std::uint64_t chain_id = 137);
