...
/// @return 32-byte HMAC
[[nodiscard]] Result<HmacSha256Hash> hmac_sha256(
    std::span<const std::uint8_t> key,
    std::span<const std::uint8_t> message);

/// Compute HMAC-SHA256 with string inputs
[[nodiscard]] inline Result<HmacSha256Hash> hmac_sha256(
    std::string_view key,
...
[[nodiscard]] Result<std::string> generate_l2_signature(
...
[[nodiscard]] inline Result<std::string> generate_builder_signature(
    std::string_view builder_secret,
    std::string_view timestamp,
    std::string_view method,
    std::string_view path,
    std::string_view body = "") {
