...
/// ecrecover precompile. Uses OpenSSL for the underlying cryptographic operations.
...
public:
    ...
    [[nodiscard]] static Result<PrivateKey> from_bytes(std::span<const std::uint8_t, 32> bytes);
    ...
    [[nodiscard]] Result<Signature> sign_message(std::span<const std::uint8_t> data) const;
...
/// Returns true if the signature was created by the private key corresponding to the address
[[nodiscard]] Result<bool> verify_signature(
    const Keccak256Hash& message_hash,
    const Signature& signature,
    const Address& expected_address);

/// Recover the signer's address from a signature
[[nodiscard]] Result<Address> recover_address(
    const Keccak256Hash& message_hash,
    const Signature& signature);
