...
[[nodiscard]] inline bool should_retry(const Error &error,
                                       const RetryPolicy &policy) noexcept {
  if (policy.retry_on_network_error &&
...
calculate_retry_delay(std::uint8_t attempt, const RetryPolicy &policy) {
  // Exponential backoff
...
///
/// @param operation Function that performs the operation and returns
/// Result<HttpResponse>
    ...
    if (result.has_value()) {
      ...
      if (should_retry(*result, policy) && attempt < policy.max_attempts) {
        std::chrono::milliseconds delay =
            calculate_retry_delay(attempt, policy);
        total_delay += delay;
