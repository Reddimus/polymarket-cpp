...
Keccak256Hash encode_string(std::string_view str) { return keccak256(str); }
...
const Keccak256Hash &eip712_domain() {
  static const Keccak256Hash hash =
...
const Keccak256Hash &clob_auth() {
  static const Keccak256Hash hash =
      keccak256("ClobAuth(address address,string timestamp,uint256 "
                "nonce,string message)");
  return hash;
}
...
const Keccak256Hash &order() {
  static const Keccak256Hash hash =
      keccak256("Order(uint256 salt,address maker,address signer,address "
                "taker,uint256 tokenId,"
                "uint256 makerAmount,uint256 takerAmount,uint256 "
                "expiration,uint256 nonce,"
...
EIP712Domain EIP712Domain::exchange(std::uint64_t chain_id) {
  return EIP712Domain{.name = "Polymarket CTF Exchange",
                      .version = "1",
                      .chain_id = chain_id,
                      .verifying_contract =
                          Address::from_hex(contracts::EXCHANGE)};
}
...
EIP712Domain EIP712Domain::neg_risk_exchange(std::uint64_t chain_id) {
  return EIP712Domain{.name = "Polymarket CTF Exchange",
                      ...
                      .chain_id = chain_id,
                      .verifying_contract =
                          Address::from_hex(contracts::NEG_RISK_EXCHANGE)};
}
...
Keccak256Hash Order::type_hash(bool neg_risk, std::uint64_t chain_id) const {
  ...
  EIP712Domain domain = neg_risk ? EIP712Domain::neg_risk_exchange(chain_id)
...
Result<Signature> sign_clob_auth(const PrivateKey &key, const ClobAuth &auth,
...
Result<Signature> sign_order(const PrivateKey &key, const Order &order,
                             bool neg_risk, std::uint64_t chain_id) {
