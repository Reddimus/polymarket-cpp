...
const Keccak256Hash& eip712_domain() {
	static const Keccak256Hash hash = keccak256("EIP712Domain(string name,string version,uint256 "
												"chainId,address verifyingContract)");
	return hash;
}

const Keccak256Hash& clob_auth() {
	static const Keccak256Hash hash = keccak256("ClobAuth(address address,string timestamp,uint256 "
...
Keccak256Hash Order::type_hash(bool neg_risk, std::uint64_t chain_id) const {
	...
	EIP712Domain domain =
		neg_risk ? EIP712Domain::neg_risk_exchange(chain_id) : EIP712Domain::exchange(chain_id);
...
Result<Signature> sign_order(const PrivateKey& key, const Order& order, bool neg_risk,
							 std::uint64_t chain_id) {
