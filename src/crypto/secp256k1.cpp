...
Result<std::array<std::uint8_t, 65>> PrivateKey::public_key() const {
	...
	size_t len = EC_POINT_point2oct(group, pub_point, POINT_CONVERSION_UNCOMPRESSED, pubkey.data(),
									pubkey.size(), nullptr);
	if (len != 65) {
...
Result<Signature> PrivateKey::sign(const Keccak256Hash& message_hash) const {
	...
	if (secp256k1_ecdsa_sign_recoverable(secp_ctx(), &recoverable, message_hash.data(),
										 impl_->raw_key.data(),
										 /* noncefp */ nullptr,
										 /* ndata   */ nullptr) != 1) {
		return std::unexpected(Error::crypto("secp256k1_ecdsa_sign_recoverable failed"));
	}

	std::array<std::uint8_t, 64> compact{};
	int recid = -1;
	if (secp256k1_ecdsa_recoverable_signature_serialize_compact(secp_ctx(), compact.data(), &recid,
																&recoverable) != 1) {
		return std::unexpected(Error::crypto("Failed to serialize recoverable signature"));
	}
	if (recid != 0 && recid != 1) {
...
Result<bool> verify_signature(const Keccak256Hash& message_hash, const Signature& signature,
...
Result<Address> recover_address(const Keccak256Hash& message_hash, const Signature& signature) {
	...
	secp256k1_ecdsa_recoverable_signature recoverable;
	if (secp256k1_ecdsa_recoverable_signature_parse_compact(secp_ctx(), &recoverable,
															compact.data(), recid) != 1) {
		return std::unexpected(Error::crypto("Failed to parse recoverable signature"));
	}

	secp256k1_pubkey pubkey;
	if (secp256k1_ecdsa_recover(secp_ctx(), &pubkey, &recoverable, message_hash.data()) != 1) {
	...
	if (secp256k1_ec_pubkey_serialize(secp_ctx(), serialized.data(), &serialized_len, &pubkey,
									  SECP256K1_EC_UNCOMPRESSED) != 1 ||
		serialized_len != 65) {
		return std::unexpected(Error::crypto("Failed to serialize recovered pubkey"));
	}
