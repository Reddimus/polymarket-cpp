...
/// IMPORTANT: Ethereum uses original Keccak (0x01 padding), not SHA-3 (0x06
/// padding).
...
// Rotation offsets
constexpr int ROTATION[5][5] = {{0, 1, 62, 28, 27},
                                {36, 44, 6, 55, 20},
                                {3, 10, 43, 25, 39},
                                {41, 45, 15, 21, 8},
                                {18, 2, 61, 56, 14}};
...
Result<Keccak256Hash> hash_from_hex(std::string_view hex) {
  ...
  if (hex.size() != 64) {
    return std::unexpected(
        Error::validation("Hash must be 32 bytes (64 hex chars)"));
  }
...
std::string to_checksum_address(const Address &addr) {
  ...
  for (size_t i = 0; i < 40; ++i) {
    ...
    std::uint8_t hash_nibble =
        (i % 2 == 0) ? (hash_byte >> 4) : (hash_byte & 0x0f);
