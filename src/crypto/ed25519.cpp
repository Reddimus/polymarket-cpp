...
Result<bool> ed25519_verify(std::span<const std::uint8_t> message,
	...
	result =
		EVP_DigestVerify(ctx, signature.data(), signature.size(), message.data(), message.size());
...
Result<std::pair<std::string, std::string>> generate_us_auth_headers(std::string_view key_id,
