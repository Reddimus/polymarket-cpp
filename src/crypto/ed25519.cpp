...
Ed25519PrivateKey::Ed25519PrivateKey(Ed25519PrivateKey &&other) noexcept =
    default;
Ed25519PrivateKey &
Ed25519PrivateKey::operator=(Ed25519PrivateKey &&other) noexcept = default;

Result<Ed25519PrivateKey>
Ed25519PrivateKey::from_seed(std::span<const std::uint8_t, 32> seed) {
  ...
  if (!key.impl_->key) {
    return std::unexpected(
        Error::crypto("Failed to create Ed25519 key from seed"));
  }
...
Result<Ed25519PrivateKey> Ed25519PrivateKey::from_hex(std::string_view hex) {
  ...
  if (hex.size() != 64) {
    return std::unexpected(
        Error::validation("Ed25519 seed must be 32 bytes (64 hex chars)"));
  }
...
Result<std::array<std::uint8_t, 32>> Ed25519PrivateKey::public_key() const {
  ...
  if (EVP_PKEY_get_raw_public_key(impl_->key, pubkey.data(), &len) != 1 ||
      len != 32) {
...
Ed25519PrivateKey::sign(std::span<const std::uint8_t> message) const {
  if (!impl_->key) {
    return std::unexpected(Error::crypto("Ed25519 key not initialized"));
  }
  ...
  result =
      EVP_DigestSign(ctx, sig.data(), &sig_len, message.data(), message.size());
  EVP_MD_CTX_free(ctx);
...
Result<bool> ed25519_verify(std::span<const std::uint8_t> message,
  ...
  if (!pkey) {
    return std::unexpected(
        Error::crypto("Failed to create Ed25519 public key"));
  }
  ...
  if (!ctx) {
    EVP_PKEY_free(pkey);
    return std::unexpected(
        Error::crypto("Failed to create verification context"));
  }
  ...
  if (result != 1) {
    ...
    EVP_PKEY_free(pkey);
    return std::unexpected(
        Error::crypto("Failed to init Ed25519 verification"));
  }
...
generate_us_auth_headers(std::string_view key_id, std::string_view secret_key,
