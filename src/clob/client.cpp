...
std::optional<std::string> extract_string(std::string_view json, std::string_view key) {
...
std::optional<int64_t> extract_int(std::string_view json, std::string_view key) {
...
struct Client::Impl {
    ...
    Result<void> add_l2_auth(http::Request& req) {
        ...
        if (pos != std::string::npos) {
            ...
            if (pos != std::string::npos) path = req.url.substr(pos);
        }
        if (path.empty()) path = "/";

        auto sig = crypto::generate_l2_signature(
            api_creds->api_secret, timestamp, method, path, req.body);
...
Result<void> Client::authenticate(
    ...
    if (!resp->is_success()) {
        return std::unexpected(Error::auth("Failed to derive API key: " + resp->body));
    }
    ...
    impl_->api_creds = ApiKeyCredentials{
        ...
        .api_passphrase = json::extract_string(resp->body, "passphrase").value_or("")
    };
...
Result<void> Client::set_builder_credentials(const BuilderKeyCredentials& credentials) {
...
Result<HealthResponse> Client::ok() {
    ...
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Health check failed"));
...
Result<ServerTimeResponse> Client::server_time() {
    ...
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Server time failed"));
...
Result<PriceResponse> Client::price(const PriceRequest& request) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/price" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get price failed"));
...
Result<std::vector<PriceResponse>> Client::prices(const std::vector<PriceRequest>& requests) {
...
Result<MidpointResponse> Client::midpoint(const MidpointRequest& request) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/midpoint" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get midpoint failed"));
    
    auto mid_str = json::extract_string(resp->body, "mid");
    return MidpointResponse{
        .mid = Decimal::from_string(mid_str.value_or("0")),
        .token_id = request.token_id
    };
}

Result<std::vector<MidpointResponse>> Client::midpoints(const std::vector<MidpointRequest>& requests) {
...
Result<SpreadResponse> Client::spread(const SpreadRequest& request) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/spread" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get spread failed"));
    
    auto spread_str = json::extract_string(resp->body, "spread");
    return SpreadResponse{
        .spread = Decimal::from_string(spread_str.value_or("0")),
        .token_id = request.token_id
    };
}

Result<std::vector<SpreadResponse>> Client::spreads(const std::vector<SpreadRequest>& requests) {
...
Result<LastTradePriceResponse> Client::last_trade_price(uint256_t token_id) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/last-trade-price" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get last trade price failed"));
...
Result<OrderBookResponse> Client::book(const OrderBookRequest& request) {
    ...
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get book failed"));
...
Result<std::vector<OrderBookResponse>> Client::books(const std::vector<OrderBookRequest>& requests) {
...
Result<TickSizeResponse> Client::tick_size(uint256_t token_id) {
    ...
    if (it != impl_->tick_size_cache.end()) {
        return TickSizeResponse{.token_id = token_id, .minimum_tick_size = it->second};
    }
    
    http::QueryBuilder query;
    query.add("token_id", key);
    
    auto resp = impl_->http.get(impl_->config.base_url + "/tick-size" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get tick size failed"));
    
    auto ts_str = json::extract_string(resp->body, "minimum_tick_size");
    TickSize ts = TickSize::Hundredth;
    if (ts_str) {
        if (*ts_str == "0.1") ts = TickSize::Tenth;
        else if (*ts_str == "0.001") ts = TickSize::Thousandth;
        else if (*ts_str == "0.0001") ts = TickSize::TenThousandth;
    }
...
Result<NegRiskResponse> Client::neg_risk(uint256_t token_id) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/neg-risk" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get neg risk failed"));
...
Result<FeeRateResponse> Client::fee_rate_bps(uint256_t token_id) {
    ...
    if (it != impl_->fee_rate_cache.end()) {
        return FeeRateResponse{.token_id = token_id, .base_fee = it->second, .maker_fee = 0, .taker_fee = it->second};
    }
    
    http::QueryBuilder query;
    query.add("token_id", key);
    
    auto resp = impl_->http.get(impl_->config.base_url + "/fee-rate-bps" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get fee rate failed"));
    ...
    return FeeRateResponse{.token_id = token_id, .base_fee = fee_rate, .maker_fee = 0, .taker_fee = fee_rate};
}

Result<PriceHistoryResponse> Client::price_history(const PriceHistoryRequest& request) {
    http::QueryBuilder query;
    query.add("token_id", request.token_id.to_decimal_string());
    if (request.interval) {
    ...
    auto resp = impl_->http.get(impl_->config.base_url + "/price-history" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get price history failed"));
    
    return PriceHistoryResponse{.history = {}};
}

Result<PaginatedResponse<MarketResponse>> Client::markets(std::optional<Cursor> cursor) {
    http::QueryBuilder query;
    if (cursor) query.add("next_cursor", cursor->value);
    
    auto resp = impl_->http.get(impl_->config.base_url + "/markets" + query.build());
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Get markets failed"));
    
    return PaginatedResponse<MarketResponse>{.items = {}, .next_cursor = std::nullopt};
}

Result<GeoblockResponse> Client::check_geoblock() {
    auto resp = impl_->http.get(impl_->config.geoblock_url + "/api/geoblock");
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Geoblock check failed"));
...
Result<std::vector<ApiKeyCredentials>> Client::api_keys() {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
...
Result<SignedOrder> Client::sign_order(const crypto::PrivateKey& key, const SignableOrder& order) {
    bool neg = impl_->neg_risk_cache.count(order.order.token_id.to_decimal_string()) > 0
...
Result<PostOrderResponse> Client::post_order(const SignedOrder& order) {
    ...
    auto resp = impl_->http.execute(req);
    if (!resp) return std::unexpected(resp.error());
    if (!resp->is_success()) return std::unexpected(Error::server("Post order failed: " + resp->body));
    
    auto order_id = json::extract_string(resp->body, "orderID");
    return PostOrderResponse{.order_id = order_id.value_or(""), .success = true};
}

Result<std::vector<PostOrderResponse>> Client::post_orders(const std::vector<SignedOrder>& orders) {
...
Result<PaginatedResponse<OpenOrderResponse>> Client::orders(
    const OrdersRequest& request,
    std::optional<Cursor> cursor) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return PaginatedResponse<OpenOrderResponse>{.items = {}, .next_cursor = std::nullopt};
}

Result<CancelOrdersResponse> Client::cancel_order(std::string_view order_id) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return CancelOrdersResponse{.canceled = {std::string(order_id)}, .not_canceled = {}};
}

Result<CancelOrdersResponse> Client::cancel_orders(const std::vector<std::string>& order_ids) {
    CancelOrdersResponse response;
    for (const auto& id : order_ids) {
        auto result = cancel_order(id);
        if (result) response.canceled.push_back(id);
        else response.not_canceled.push_back(id);
    }
    return response;
}
...
Result<CancelOrdersResponse> Client::cancel_market_orders(
...
Result<PaginatedResponse<TradeResponse>> Client::trades(
    const TradesRequest& request,
    std::optional<Cursor> cursor) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return PaginatedResponse<TradeResponse>{.items = {}, .next_cursor = std::nullopt};
}

Result<BalanceAllowanceResponse> Client::balance_allowance(const BalanceAllowanceRequest& request) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return BalanceAllowanceResponse{};
}

VoidResult Client::update_balance_allowance(const BalanceAllowanceRequest& request) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return {};
}
...
VoidResult Client::delete_notifications(const std::vector<std::string>& notification_ids) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return {};
}

Result<OrderScoringResponse> Client::is_order_scoring(std::string_view order_id) {
    if (!impl_->authenticated) return std::unexpected(Error::auth("Not authenticated"));
    return OrderScoringResponse{};
}

Result<std::vector<OrderScoringResponse>> Client::are_orders_scoring(
...
Result<HeartbeatResponse> Client::post_heartbeat(std::optional<std::string> heartbeat_id) {
    if (!impl_->is_builder_mode) return std::unexpected(Error::auth("Builder credentials not set"));
...
const std::string& Client::host() const noexcept { return impl_->config.base_url; }
const ClientConfig& Client::config() const noexcept { return impl_->config; }
std::optional<Address> Client::signer_address() const noexcept { return impl_->signer_addr; }
std::optional<Address> Client::funder_address() const noexcept { return impl_->funder_addr; }
