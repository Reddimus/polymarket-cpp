...
inline std::uint64_t mul_div_u64(std::uint64_t a, std::uint64_t b, std::uint64_t divisor) {
	...
	return static_cast<std::uint64_t>((static_cast<unsigned __int128>(a) * b) / divisor);
...
std::pair<uint256_t, uint256_t> calculate_limit_amounts(Side side, const Decimal& price,
														const Decimal& size) {
...
Result<SignableOrder> OrderBuilder<Limit>::build() const {
	...
	if (!price_) {
		return std::unexpected(Error::validation("price is required for limit orders"));
	}
	if (!size_) {
		return std::unexpected(Error::validation("size is required for limit orders"));
	}

	// Validate price bounds
	if (*price_ < Decimal::from_double(0.001) || *price_ > Decimal::from_double(0.999)) {
		return std::unexpected(Error::validation("price must be between 0.001 and 0.999"));
	}

	// Calculate amounts
	auto [maker_amount, taker_amount] = calculate_limit_amounts(*side_, *price_, *size_);
	...
	signable.order.expiration = expiration_.value_or(0) > 0
...
Result<SignableOrder> OrderBuilder<Market>::build() const {
	...
	if (!amount_) {
		return std::unexpected(Error::validation("amount is required for market orders"));
	}

	// Market orders need a price for calculation
	Decimal effective_price = price_.value_or((*side_ == Side::Buy) ? Decimal::from_double(0.999)
	...
	signable.order.maker_amount = maker_amount;
	signable.order.taker_amount = taker_amount;
	signable.order.expiration = default_expiration(); // Short expiration for market orders
	signable.order.nonce = uint256_t::from_uint64(nonce_.value_or(0));
	signable.order.fee_rate_bps = uint256_t::from_uint64(fee_rate_);
	signable.order.side = static_cast<uint8_t>(*side_);
	signable.order.signature_type = static_cast<uint8_t>(signature_type_);
