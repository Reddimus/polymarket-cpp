...
std::pair<uint256_t, uint256_t> calculate_limit_amounts(
    Side side,
    const Decimal& price,
    const Decimal& size) {
...
Result<SignableOrder> OrderBuilder<Market>::build() const {
    ...
    Decimal effective_price = price_.value_or(
        (*side_ == Side::Buy) ? Decimal::from_double(0.999) : Decimal::from_double(0.001)
    );
